...
(function(){
  setTimeout(function(){
      ...
      if(mtSec){
        ...
        if(mtCapsBox&&typeof gsap!=='undefined'){
...
(function(){
  ...
  if(hdr){
    ...
    hdr.querySelectorAll('p').forEach(function(p){p.style.cssText='font-size:1.1rem;color:rgba(255,255,255,0.75);line-height:1.7;max-width:600px;margin:0 auto';
      if(p.textContent.indexOf('Autonomi')!==-1){p.innerHTML=p.innerHTML.replace('Autonomi','<span style="color:#C9A84C;font-weight:700">Autonomi</span>')}
    });
    /* Electric-blue badge */
    var badge=hdr.querySelector('.sc-badge');
    if(badge){badge.style.borderColor='#C9A84C';badge.style.color='#C9A84C';
      var bImg=badge.querySelector('img');
      // Shift blue AI icon toward the gold theme color via filter (no gold asset exists)
      if(bImg){bImg.src='https://lynz-tonomi.github.io/macrobrands/AI-small-blue.png';bImg.style.height='22px';bImg.style.width='auto';bImg.style.filter='hue-rotate(-150deg) saturate(0.55) brightness(1.05)'}
    }
  }
  ...
  if(hdr){
    ...
    circBg.style.cssText='position:absolute;inset:0;background:url(https://lynz-tonomi.github.io/macrobrands/blue-circuite2.png) calc(50% + 2px) calc(47% - 13px)/cover no-repeat;opacity:0;z-index:0;pointer-events:none;filter:hue-rotate(-150deg) saturate(0.55) brightness(1.05)';
    ...
    var moduleData=[
      {name:'SOURCING',sub:'Vendor Discovery',icon:'\u{1F50D}',x:8.4,y:3.1,color:'#C9A84C'},
      {name:'LOGISTICS',sub:'Route Optimization',icon:'\u{1F69A}',x:91.6,y:2.0,color:'#C9A84C'},
      ...
      {name:'ANALYTICS',sub:'Data Intelligence',icon:'\u{1F4C8}',x:92.5,y:53.8,color:'#C9A84C'},
      ...
      {name:'TRACEABILITY',sub:'Batch Tracking',icon:'\u{1F517}',x:68.8,y:90.0,color:'#C9A84C'}
    ];
      ...
      setTimeout(function(){
        ...
        if(chipSvg){
          ...
          paths.forEach(function(p,pi){
            ...
            var isBlue=p.getAttribute('stroke')==='#C9A84C';
            var color=isBlue?'#C9A84C':'#fff';
            var len=p.getTotalLength();
            if(len<80)return;
