  ...
  if(hdr){
    ...
    if(typeof gsap!=='undefined'&&typeof ScrollTrigger!=='undefined'){
      setTimeout(function(){
        ...
        var nodes=scFlow.querySelectorAll('.ai-node-w');
        gsap.to(nodes,{opacity:1,ease:'none',stagger:.015,scrollTrigger:{trigger:scFlow,start:'top 55%',end:'top -30%',scrub:true}});
        // Blue filled nodes appear
        var blueDots=scFlow.querySelectorAll('.ai-node-b');
        gsap.to(blueDots,{opacity:1,ease:'none',stagger:.015,scrollTrigger:{trigger:scFlow,start:'top 55%',end:'top -30%',scrub:true}});
        // Junction dots
        var juncs=scFlow.querySelectorAll('.ai-junc');
        gsap.to(juncs,{opacity:1,ease:'none',stagger:.006,scrollTrigger:{trigger:scFlow,start:'top 75%',end:'top -20%',scrub:true}});
        // Labels fade in
        var lbls=scFlow.querySelectorAll('.ai-label');
        gsap.to(lbls,{opacity:1,ease:'none',stagger:.03,scrollTrigger:{trigger:scFlow,start:'top 40%',end:'top -10%',scrub:true}});
        // Blinking endpoint nodes
        scFlow.querySelectorAll('.ai-node-w,.ai-node-b').forEach(function(n){
          gsap.to(n,{opacity:.15,duration:.8+Math.random()*.6,repeat:-1,yoyo:true,ease:'sine.inOut',delay:Math.random()*2});
        });
        scFlow.querySelectorAll('.ai-junc').forEach(function(n){
          gsap.to(n,{opacity:.1,duration:1+Math.random()*.8,repeat:-1,yoyo:true,ease:'sine.inOut',delay:Math.random()*3});
        });
      },200);
